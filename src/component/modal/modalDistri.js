...
const ModalDistri = () => {
  ...
  return (
    <ModalForm
      ...
      y
      trigger={<Button type="dashed">Phân tán dữ liệu</Button>}
      form={form}
      autoFocusFirstInput
      modalProps={{
        destroyOnClose: true,
    ...
    >
      {isLogin === false && (
        <ProForm.Group>
          ...
          <Button type="primary" onClick={loginMysql}>
            Đăng nhập
          </Button>
      ...
      {isLogin === true && (
        <>
          ...
          <Card title="Truy vấn" bordered={false} style={{ width: "100%" }}>
            Select <strong>{`${bang ? bang : ""}`}.*</strong> from{" "}
            <strong>{`${bang ? bang : ""}`}</strong> {" where "}
            <strong>{`${cot ? cot : ""}`}</strong> ={" "}
            <strong>{`'${dieukien ? dieukien : ""}'`}</strong>
          </Card>
        </>
      )}
    </ModalForm>
